...
class User:
    ...
    @classmethod
    def save(cls, data):
        query = "INSERT INTO users (first_name, last_name, email, phone_number, gender, password) VALUES (%(first_name)s, %(last_name)s, %(email)s, %(phone_number)s, %(gender)s, %(password)s);"
    ...
    def validate_register(user):
        ...
        if len(user['last_name']) < 3:
            ...
            is_valid= False
        if len(str(user['phone_number'])) < 8:
            flash("The phone number must contain at least 8 numbers","register")
            is_valid= False
        if not user['gender']:
            flash("You must select your gender","register")
            is_valid= False
